...
char	*ft_final_buff(char *buff, int fd, char *final, char **g_rb)
{
	int		i;
	int		b_read;
	char	*temp;
	...
	while (b_read > 0)
	{
		...
		if (buff[i] == '\n')
		{
			temp = ft_substr(buff, 0, i + 1);
			final = ft_free(final, ft_strjoin(final, temp));
			*g_rb = ft_free(*g_rb, ft_substr(buff, i + 1, i - b_read - 1));
			free(temp);
			break ;
		}
		final = ft_free(final, ft_strjoin(final, buff));
		b_read = read(fd, buff, BUFFER_SIZE);
	}
	if (b_read < 0 || (b_read == 0 && final[0] == '\0'))
		return (NULL);
	return (final);
}
...
int	ft_rest_read(char **g_rb, char **final_buff)
{
	...
	while ((*g_rb)[i] != '\0')
	{
		...
		{
			control = 1;
			*final_buff = ft_free(*final_buff, ft_substr(*g_rb, 0, i + 1));
			len = ft_strlen(*g_rb);
			*g_rb = ft_free(*g_rb, ft_substr(*g_rb, i + 1, len - (i + 1)));
			break ;
		}
		i++;
	}
...
char	*final_check(char *temp, char **final, char **g_rb)
{
	if (!temp)
	{
		free(*final);
...
char	*get_next_line(int fd)
{
	...
	if (!g_re_buff[fd])
	{
		g_re_buff[fd] = ft_calloc(BUFFER_SIZE + 1, sizeof(char));
		if (!g_re_buff[fd])
			return (NULL);
	}
	if (g_re_buff[fd][0] != '\0')
	{
		control = ft_rest_read(&g_re_buff[fd], &final_buff);
		temp = final_buff;
	}
	if (g_re_buff[fd][0] == '\0' && control == 0)
		temp = ft_no_rest_read(&g_re_buff[fd], final_buff, fd);
	return (final_check(temp, &final_buff, &g_re_buff[fd]));
}
